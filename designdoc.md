...
### NFR-4: Observability
- Structured JSON logging (same as job scheduler: `asctime`, `levelname`, `message`)
...
### NFR-5: Reliability
- Layered architecture: Routes → Service → Repository (same pattern as job scheduler)
...
- `scripts/rebuild.sh` and `scripts/deploy.sh` same pattern as job scheduler
